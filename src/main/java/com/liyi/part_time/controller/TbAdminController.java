...
import com.liyi.part_time.util.PasswordUtil;
...
public class TbAdminController extends BaseController<TbAdmin> {
    ...
    public Result register(HttpServletRequest  request, String adminName, String adminPsw, String phone, String code, String adminSex,Integer state,String entry_time) {
        ...
        // 加密+加盐
        String pswwithMD5andsalt = p.generate(adminPsw);
        tbAdmin.setAdminPsw(pswwithMD5andsalt);
        System.out.print(pswwithMD5andsalt);
...
//        tbAdmin.setAdminPsw(pswwithMD5andsalt);
        ...
        return ResultFactory.buildSuccessResult("注册成功", tbAdmin);
    }
...
///  结合用户名、密码和验证码进行登录验证
    ...
    public Result verificationWithResult(String username,String password,String captcha) throws IOException {
        ...
        PasswordUtil p = new PasswordUtil();
//        String pswwithMD5andsalt = p.generate(password);
//        tbAdmin.setAdminPsw(pswwithMD5andsalt);
//        Mademd5 md5 = new Mademd5();//new一个MD5对象
...
//        else if(!password.equals( tbAdmin.getAdminPsw()))
        else if(!p.verify(password, tbAdmin.getAdminPsw()))
        {
...
//        if(!old_password.equals(tbAdmin.getAdminPsw()))
        if(!p.verify(old_password, tbAdmin.getAdminPsw()))
        {
...
//            tbAdmin.setAdminPsw(newpswwithMD5andsalt);
            String newpswwithMD5andsalt = p.generate(new_password);
            tbAdmin.setAdminPsw(newpswwithMD5andsalt);
